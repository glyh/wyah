...
let rec free_variables (scheme : wyah_type) : type_var_set =
  ...
  | TArrow (lhs, rhs) ->
      TypeVarSet.union (free_variables lhs) (free_variables rhs)
...
let rec apply_sub_ty (s : subst) (ty : wyah_type) : wyah_type =
  ...
  | TVar v -> (
      match MapSubs.find_opt v s with Some to_sub -> to_sub | None -> TVar v)
  | TArrow (lhs, rhs) -> TArrow (apply_sub_ty s lhs, apply_sub_ty s rhs)
  | TIO inner -> TIO (apply_sub_ty s inner)
  | t -> t
...
let rec unify_one (c : type_constraint) :
    (tvar * wyah_type) option * type_constraint list =
  match c with
  | lhs, rhs when compare lhs rhs == 0 -> (None, [])
  | TForall (vs1, inner1), TForall (vs2, inner2) ->
      if compare vs1 vs2 == 0 then unify_one (inner1, inner2)
      else raise (UnificationFailure c)
  | TVar v, ty | ty, TVar v ->
      let fvs_rhs = free_variables ty in
      if TypeVarSet.mem v fvs_rhs then raise (UnificationFailure c)
      else (Some (v, ty), [])
  | TArrow (l1, l2), TArrow (r1, r2) -> (None, [ (l1, r1); (l2, r2) ])
  | TIO lhs, TIO rhs -> unify_one (lhs, rhs)
  | _ -> raise (UnificationFailure c)

let apply_sub_sub (s_applied : subst) (s_target : subst) : subst =
  s_target |> MapSubs.map (apply_sub_ty s_applied)
...
let rec unify (cs : type_constraint list) : subst =
  ...
  | [] -> MapSubs.empty
  | c :: cs -> (
      match unify_one c with
      | Some (s_v, s_t), cs_new ->
          MapSubs.add s_v s_t
            (unify (cs_new @ apply_sub_cs (MapSubs.singleton s_v s_t) cs))
      | None, cs_new -> unify (cs_new @ cs))

let generalize (cs : type_constraint list) (env : type_env) (var : identifier)
    (ty : wyah_type) : type_env =
...
let rec inference_constraints (tenv : type_env) (exp : expr) :
    wyah_type * type_constraint list =
  match exp with
  | Atom Unit -> (t_unit, [])
  | Atom (Int _) -> (t_int, [])
  | Atom (Char _) -> (t_char, [])
  ...
  | Var var -> (
      match TypeEnv.find_opt var tenv with
      | None -> raise (UndefinedVariable var)
      | Some scheme -> (instantiate scheme, []))
  | Lam (var, body) ->
      let var_gen = TVar (gen_tvar ()) in
      let tenv_new = TypeEnv.add var var_gen tenv in
      let body_type, constraints = inference_constraints tenv_new body in
  ...
  | If (cond, then_clause, else_clause) ->
      ...
      ( then_ty,
        [ (cond_ty, t_bool); (then_ty, else_ty) ]
...
let show_sub (smap : subst) : string =
  MapSubs.fold
    (fun k v acc -> string_of_int k ^ " -> " ^ pretty_print_type v ^ "\n" ^ acc)
    smap ""
...
let inference_type (tenv : type_env) (exp : expr) : wyah_type =
  ...
  if compare rest_fvs TypeVarSet.empty == 0 then type_sub
