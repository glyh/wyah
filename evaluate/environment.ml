...
let equal_value (lhs : value) (rhs : value) : value =
  match (lhs, rhs) with
  | Norm a, Norm b -> Norm (Bool (compare a b == 0))
  | _ -> raise Unreachable
...
let eval_env_init : eval_env =
  ...
  |> List.to_seq |> EvalEnv.of_seq

let type_env_init : type_env =
  env_init
  |> List.map (fun (id, _, ty) -> (id, ty))
  |> List.to_seq |> TypeEnv.of_seq
